...
std::optional<std::string> SNIExtractor::extract(const uint8_t* payload, size_t length) {
    ...
    while (offset + 4 <= extensions_end) {
        ...
        // Don't parse ouside the extension area
        ...
        if (extension_type == EXTENSION_SNI) {
            ...
            // Extract the hostname and it becomes a string
...
std::vector<std::pair<uint16_t, std::string>> SNIExtractor::extractExtensions(
    ...
    // First make sure this is a TLS Client Hello
    if (!isTLSClientHello(payload, length)) {
        return extensions;
    }
    ...
    // Make sure ClientHello has at least:
    // 2 bytes version + 32 bytes random
    if (offset + 34 > length) {
        return extensions;
    }
    ...
    if (offset + cipher_suites_length > length) {
        return extensions;
    }
    ...
    // Make sure the complete extensions block exists
    ...
    while (offset + 4 <= extensions_end) {
        ...
        // Make sure extension data fits
        if (offset + extension_length > extensions_end) {
            break;
        }

        // Convert extension data into a string
        ...
        // Store:
        // pair.first  = extension type
        // pair.second = raw extension data
...
std::optional<std::string> HTTPHostExtractor::extract(const uint8_t* payload, size_t length) {
    ...
    // Once HTTP is confirmed then Search for "Host: " header
